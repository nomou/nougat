...
public class CharacterEncodingFilter extends OncePerRequestFilter {
     ...
     */
    @Override
    protected void doFilterInternal(final ServletRequest httpRequest, final ServletResponse httpResponse,
                                    final FilterChain chain) throws IOException, ServletException {
        if (httpRequest instanceof HttpServletRequest && httpResponse instanceof HttpServletResponse) {
            this.doFilterInternal((HttpServletRequest) httpRequest, (HttpServletResponse) httpResponse, chain);
        } else {
    ...
    private static Map<String, String[]> parseQueryString(final String queryString,
        ...
        if (log.isTraceEnabled()) {
            log.trace("parse query string: {}", query);
        }
        ...
        if (!urlEncoded && !unescape) {
            if (log.isTraceEnabled()) {
                log.trace("no encoded or unescape char for query string: {}", query);
            }
            ...
            if (log.isTraceEnabled()) {
                log.trace("detected charset is {} for query string: {}", charset, queryString);
            }
    ...
    private static Map<ByteBuffer, List<ByteBuffer>> doParseUrlEncodedQueryString(String[] params, CharsetCounter[] counters) {
        if (log.isTraceEnabled()) {
            log.trace("parse url encoded params: {}", Arrays.toString(params));
        }
    ...
    private static Map<ByteBuffer, List<ByteBuffer>> doParseUnescapeQueryString(final String[] params,
        ...
        if (log.isTraceEnabled()) {
            log.trace("parse unescape params: {}", Arrays.toString(params));
        }
