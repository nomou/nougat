...
public abstract class MultipartParser {
    ...
    public interface Consumer {
        ...
        void consumeParameter(final String name, final String value, final Map<String, String> multipartHeaders);

        void consumePart(final String name, final String filename, final InputStream multipartIn, final Map<String, String> multipartHeaders) throws IOException;
     ...
     * @param consumer     Multipart 处理器
     * @throws IOException
     */
    @SuppressWarnings("unchecked")
    public static void parse(final HttpServletRequest httpRequest, final Consumer consumer) throws IOException {
        ...
        if (null != SPRING_MULTIPART && SPRING_MULTIPART.isInstance(httpRequest)) {
            doHandle(httpRequest, consumer);
        } else {
            doParse(httpRequest, consumer);
        }
    }

    @SuppressWarnings("unchecked")
    private static void doHandle(final HttpServletRequest httpRequest, final Consumer consumer) throws IOException {
        ...
        for (Map.Entry<String, String[]> entry : set) {
            ...
            for (final String value : entry.getValue()) {
                consumer.consumeParameter(name, value, request.getMultipartHeaders(name).toSingleValueMap());
            }
        }
        for (final Map.Entry<String, MultipartFile> entry : fileMap.entrySet()) {
            ...
            try {
                consumer.consumePart(name, value.getOriginalFilename(), in, request.getMultipartHeaders(name).toSingleValueMap());
            } finally {
                IOUtils.close(in);
            }
        }
     ...
     * \r\n--boundary--
     */
    private static void doParse(final HttpServletRequest request, Consumer consumer) throws IOException {
        ...
        if (next(input, boundary, true)) {
            do {
                doParsePart(input, boundary, charset, consumer);
     ...
     * @throws IOException
     */
    private static void doParsePart(InputStream in, byte[] boundary, String defaultCharset, Consumer consumer) throws IOException {
        ...
        try {
            if (null != consumer) {
                // form field
                if (null == filename) {
                    final String value = IOUtils.toString(pin, Charset.forName(charset), true);
                    consumer.consumeParameter(name, value, headers);
                } else {
                    consumer.consumePart(name, filename, pin, headers);
                }
            }
        } finally {
            IOUtils.close(pin);
