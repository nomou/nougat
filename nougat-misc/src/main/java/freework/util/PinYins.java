...
public abstract class PinYins {
    ...
    public static String getAcronym(String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            sb.append(getAcronym(text.charAt(i)));
        }
        return sb.toString();
    }
    ...
    /**
     * 获取一个中文的拼音首字符
     * 通过GB2312编码规则, 将中文转换为相应的开头字母, 简单实现, 多音只能取第一个
     * 中文字符只能是GB2312编码中含有的中文字符
     */
    public static char getAcronym(char c) {
        // 如果不是中文字符则直接返回
        if (!(c + "").matches("[\\u4E00-\\u9FA0]{1}")) {
            return c;
        }

        // GB2312使用2个字节存放,获取GB2312编码的二进制值
        byte[] bytes = (c + "").getBytes(GB2312);
        int asc = ((bytes[0] & 0xFF) << 8) + (bytes[1] & 0xFF) - 0x10000;
     ...
     * @param acronym 是否首字母
     */
    public static String getPinYin(String text, boolean acronym) {
        if (null == text) {
            return null;
        }
        ...
        for (int i = 0; i < bytes.length; i++) {
            ...
            // US-ASCII
            while (b < 0x80 && b >= 0) {
                if (Character.isLetter((char) b) || Character.isDigit((char) b)) {
                    strValue = (strValue + (char) bytes[i]).toUpperCase();
                } else {
                    if (strValue.length() > 0) {
                        letter.append(strValue);
                    }
                    strValue = "";
                }
            ...
            ucHigh = (short) (bytes[i] + 256);
            System.out.println(ucHigh + "-" + bytes[i]);
            ucLow = (short) (bytes[i + 1] + 256);
            System.out.println(ucLow + "-" + bytes[i + 1]);
            ...
            String strRes = findFullPinYin(nCode);
            letter.append(acronym ? strRes.charAt(0) + "" : strRes);
            i++;
        }
     ...
     * @return String
     */
    protected static String findFullPinYin(int code) {
        String value = "";
        switch (code) {
            case 6325:
            case 6436:
    ...
    public static void main(String[] args) throws Throwable {
        String 大众 = PinYins.getAcronym("大众");
