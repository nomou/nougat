...
public class IPRange {
     ...
     * @throws IllegalArgumentException Throws this exception when the specified string doesn't represent a valid IP address.
     */
    ...
    final void parseRange(String range) {
        ...
        if (index == -1) {
            ipAddress = IPAddress.parse(range);
        } else {
            ipAddress = IPAddress.parse(range.substring(0, index));
        ...
        try {
            ...
            ipSubnetMask = IPAddress.parse(subnetStr);
    ...
    private IPAddress computeMaskFromNetworkPrefix(int prefix) {
        ...
        return IPAddress.parse(decimalString);
    ...
    public boolean isIPAddressInRange(IPAddress address) {
        ...
        int result1 = address.getAddress() & ipSubnetMask.getAddress();
        int result2 = ipAddress.getAddress() & ipSubnetMask.getAddress();
