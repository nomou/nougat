...
class Restarter {
    ...
    static {
        ...
        if (Platform.isWindows() && is64Bit) {
            RESTARTER = makeExecutable(Unpacker.unpack(Restarter.class.getResource("/bin/windows/x86_64/restarter.exe")));
        } else if (Platform.isMac() && is64Bit) {
            RESTARTER = makeExecutable(Unpacker.unpack(Restarter.class.getResource("/bin/macosx/x86_64/restarter")));
        } else {
            RESTARTER = null;
        }
    }
